...
/*
 syntax of a reducer :-  (state,action) => newState
  ...
  ex:- const initialState = { isLoading: false,    items: [] };
...
**Redux :-  it is used as a state management tool with React and it is an open-source JavaScript library used to manage application state. React uses Redux for building the user interface .you can use it with any other JavaScript framework or library it creates a global state for the whole application, that can be accessed by any of your components. Redux is a predictable state container for JavaScript apps. It helps you write applications that behave consistently, run in different environments (client, server, and native), and are easy to test.
...
action which describes what changes we want to make in the state. These actions are then consumed by something known as **reducers**, which are pure functions that take the current state and the action and return a new state. This new state is then stored in the store and is available to all the components in the application.

**Reducer :-  reducer is a function that is able to process our message, our Action. it take in two things: previous state and an action. It then processes the action and returns a new state. It is a pure function, meaning it does not modify the state, but then they reduce it  to one entity: the new updated instance of state. So reducers are basically pure JS functions which take in the previous state and an action and return the newly updated state.

2 Whenever an action is dispatched, all the reducers are activated. Each reducer filters out the action using a switch statement switching on the action type. Whenever the switch statement matches with the action passed, the corresponding reducers take the necessary action to make the update and return a fresh new instance of the global state. This new state is then stored in the store and is available to all the components in the application.
...
Similarly, removing an item from the cart. The application has to internally handle that action by removing that item from the cart object
...
states. It will store the state of the cart component, the state of the user's profile, the state of the user's orders, and so on. It will
*/